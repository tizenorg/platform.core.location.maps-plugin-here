...
here_error_e HereView::Init(map_view_h hView, maps_plugin_map_view_ready_cb pCbFunc)
{
	if (!hView)
	...
	do {
		error2 = map_view_get_viewport(hView, &m_pImpl->img);
		if (error2 != MAPS_ERROR_NONE) break;

		error2 = map_view_get_screen_location(hView, &m_pImpl->x, &m_pImpl->y, &m_pImpl->w, &m_pImpl->h);
		if (error2 != MAPS_ERROR_NONE) break;
...
void HereView::__setMapType()
{
	...
	map_type_e map_type;
	map_view_get_map_type(m_pImpl->hView, &map_type);

	bool buildings_enabled = false;
	map_view_get_buildings_enabled(m_pImpl->hView, &buildings_enabled);

	bool traffic_enabled = false;
	map_view_get_traffic_enabled(m_pImpl->hView, &traffic_enabled);

	bool public_transit_enabled = false;
	map_view_get_public_transit_enabled(m_pImpl->hView, &public_transit_enabled);
	...
	const double dZoom, const double dAngle)
{
	...
	/* resize window */
	int x, y, w, h;
	map_view_get_screen_location(m_pImpl->hView, &x, &y, &w, &h);
	m_pImpl->w = MAX(m_pImpl->w, 1);
	m_pImpl->h = MAX(m_pImpl->h, 1);
...
here_error_e HereView::ScreenToGeolocation(const int x, const int y, maps_coordinates_h *mapsCoord)
{
...
here_error_e HereView::OnViewObject(const map_object_h object, const map_object_operation_e operation)
{
	if (!m_pImpl || !m_pImpl->isInitialized || !m_pImpl->map)
		return HERE_ERROR_SERVICE_NOT_AVAILABLE;

	if (!object || operation < MAP_OBJECT_ADD || operation > MAP_OBJECT_REMOVE)
		return HERE_ERROR_INVALID_PARAMETER;
...
void HereView::__processViewObject(const map_object_h object, const map_object_operation_e operation)
{
	map_object_type_e type = MAP_OBJECT_UNKNOWN;
	map_object_get_type(object, &type);

	if (type < MAP_OBJECT_POLYLINE || type > MAP_OBJECT_UNKNOWN) return;
	if (operation < MAP_OBJECT_ADD || operation > MAP_OBJECT_REMOVE) return;
	...
	MAPS_LOGD("type=%s, operation=%s, object=%p",
		(type >= MAP_OBJECT_POLYLINE && type <= MAP_OBJECT_UNKNOWN) ? type_str[type] : "?",
		(operation >= MAP_OBJECT_ADD && operation <= MAP_OBJECT_REMOVE) ? oper_str[operation] : "?",
		object);
