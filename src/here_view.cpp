...
here_error_e HereView::OnViewObject(const map_object_h object, map_object_operation_e operation)
{
	if (!m_pImpl || !m_pImpl->isInitialized || !m_pImpl->map)
		return HERE_ERROR_SERVICE_NOT_AVAILABLE;
...
void HereView::__processViewObject(const map_object_h object, map_object_operation_e operation)
{
	map_object_type_e type;
	map_object_get_type(object, &type);

	if (type < MAP_OBJECT_POLYLINE || type > MAP_OBJECT_MARKER) return;
	...
	MAPS_LOGD("type=%s, operation=%s, object=%p",
		(type >= MAP_OBJECT_POLYLINE && type <= MAP_OBJECT_MARKER) ? type_str[type] : "?",
