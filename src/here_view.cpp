...
here_error_e HereView::Init(map_view_h hView)
{
	...
	do {
		error2 = map_view_get_panel(hView, &m_pImpl->img);
...
here_error_e HereView::RenderMap(const maps_coordinates_h mapsCoord,
	const double dZoom, const double dAngle, const double dTilt)
{
...
here_error_e HereView::RenderMapByArea(const maps_area_h hArea,
	const double dZoom, const double dAngle, const double dTilt)
{
...
here_error_e HereView::GetMinTilt(int *nMinTilt)
{
	if (!m_pImpl || !m_pImpl->isInitialized || !m_pImpl->map)
		return HERE_ERROR_SERVICE_NOT_AVAILABLE;

	//*nMinTilt = m_pImpl->map->GetMinTilt();
	return HERE_ERROR_NONE;
}

here_error_e HereView::GetMaxTilt(int *nMaxTilt)
{
	if (!m_pImpl || !m_pImpl->isInitialized || !m_pImpl->map)
		return HERE_ERROR_SERVICE_NOT_AVAILABLE;

	//*nMaxTilt = m_pImpl->map->GetMaxTilt();
	return HERE_ERROR_NONE;
}

here_error_e HereView::OnViewObject(const map_view_object_h object,
				const map_view_object_operation_e operation)
{
	if (!m_pImpl || !m_pImpl->isInitialized || !m_pImpl->map)
		return HERE_ERROR_SERVICE_NOT_AVAILABLE;

	if (!object || operation < MAP_VIEW_OBJECT_ADD || operation > MAP_VIEW_OBJECT_REMOVE_ALL)
		return HERE_ERROR_INVALID_PARAMETER;
...
Eina_Bool HereView::__idlerCb(void *data)
{
	...
	while (pImpl->pendingObjects.size())
	{
		...
		map_view_object_h object = pending.first;
		map_view_object_operation_e operation = pending.second;
...
void HereView::__processViewObject(const map_view_object_h object,
				const map_view_object_operation_e operation)
{
	map_view_object_type_e type = MAP_VIEW_OBJECT_UNKNOWN;
	map_view_object_get_type(object, &type);

	if (type < MAP_VIEW_OBJECT_LAYER || type > MAP_VIEW_OBJECT_UNKNOWN) return;
	if (operation < MAP_VIEW_OBJECT_ADD || operation > MAP_VIEW_OBJECT_REMOVE_ALL) return;

	const char *oper_str[20] = { "ADD", "SET_VISIBLE", "MOVE", "CHANGE", "REMOVE", "REMOVEALL"};
	const char *type_str[20] = { "LAYER", "POLYLINE", "POLYGON", "MARKER", "ROUTE", "UNKNOWN"};

	MAPS_LOGD("type=%s, operation=%s, object=%p",
		(type >= MAP_VIEW_OBJECT_LAYER && type <= MAP_VIEW_OBJECT_UNKNOWN) ? type_str[type] : "?",
		(operation >= MAP_VIEW_OBJECT_ADD && operation <= MAP_VIEW_OBJECT_REMOVE_ALL) ? oper_str[operation] : "?",
		object);

	switch(operation)
	{
	case MAP_VIEW_OBJECT_ADD:         m_pImpl->visualObjects.add(object); break;
	case MAP_VIEW_OBJECT_SET_VISIBLE: m_pImpl->visualObjects.setVisible(object); break;
	case MAP_VIEW_OBJECT_MOVE:        m_pImpl->visualObjects.move(object); break;
	case MAP_VIEW_OBJECT_CHANGE:      m_pImpl->visualObjects.update(object); break;
	case MAP_VIEW_OBJECT_REMOVE:      m_pImpl->visualObjects.remove(object); break;
	case MAP_VIEW_OBJECT_REMOVE_ALL:  m_pImpl->visualObjects.removeAll(); break;
	default:                          break;
	}
}
